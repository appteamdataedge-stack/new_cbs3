/**
 * Statement of GL Module
 * Allows users to generate GL statements for any GL account with a maximum 6-month date range
 */
...
import type { GLOption } from '../types/glStatement.types';
import { getGLList, generateGLStatement } from '../services/glStatementService';
...
const StatementOfGL: React.FC = () => {
  // State variables
  const [selectedGL, setSelectedGL] = useState<string>('');
  const [selectedGLData, setSelectedGLData] = useState<GLOption | null>(null);
  ...
  // Fetch GL account list on component mount
  useEffect(() => {
    const fetchGLAccounts = async () => {
      setIsLoading(true);
      setError(null);
      try {
        const glList = await getGLList();
        setGLAccounts(glList);
      } catch (err: any) {
        setError(err.message || 'Failed to load GL account list');
        toast.error('Failed to load GL account list');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Handler: GL account selection change
  const handleGLChange = (selectedOption: any) => {
    if (selectedOption) {
      setSelectedGL(selectedOption.value);
      const glData = glAccounts.find(gl => gl.glNum === selectedOption.value);
      setSelectedGLData(glData || null);
    } else {
      setSelectedGL('');
      setSelectedGLData(null);
    }
    setError(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await generateGLStatement(selectedGL, fromDate, toDate, format);
      toast.success('GL Statement generated successfully');
    } catch (err: any) {
      toast.error(err.message || 'Failed to generate GL statement');
    } finally {
  ...
  const glOptions = glAccounts.map(gl => ({
    value: gl.glNum,
    label: `${gl.glNum} - ${gl.glName} (${gl.currency})`
  }));

  // Check if form is valid
  const isFormValid = selectedGL && fromDate && toDate && !dateRangeError && !isGenerating;

  return (
    <Box>
      <PageHeader
        title="Statement of GL"
        subtitle="Generate GL statements with a maximum 6-month date range"
      ...
      <form onSubmit={handleSubmit}>
        <FormSection title="GL Statement Information" compact>
          <Grid container spacing={2}>
            {/* GL Account Number Field */}
            <Grid item xs={12}>
              <FormControl fullWidth>
                <Typography variant="body2" gutterBottom sx={{ fontWeight: 'medium', mb: 0.5, fontSize: '0.875rem' }}>
                  GL Account <span style={{ color: 'error.main' }}>*</span>
                </Typography>
                {isLoading ? (
            ...
            {selectedGLData && (
              <Grid item xs={12} md={6}>
                ...
                <TextField
                  fullWidth
                  size="small"
                  value={selectedGLData.glName}
                  disabled
                  InputProps={{
                    style: { fontSize: '0.875rem', backgroundColor: '#f5f5f5' },
                  }}
                />
              </Grid>
            )}

            {/* Auto-filled Currency */}
            {selectedGLData && (
              <Grid item xs={12} md={6}>
                <Typography variant="body2" gutterBottom sx={{ fontWeight: 'medium', mb: 0.5, fontSize: '0.875rem' }}>
                  Currency
