-- Fixed procedure with proper error handling
USE moneymarketdb;
...
BEGIN
    ...
    -- Only trigger done on cursor fetch, not on SELECT statements
    ...
    read_loop: LOOP
        ...
        IF v_account_no != v_current_account THEN
            ...
            SELECT IFNULL(Current_Balance, 0) INTO v_previous_balance
            FROM acct_bal
        ...
        ELSE
            ...
            SELECT IFNULL(BALANCE_AFTER_TRAN, 0) INTO v_previous_balance
            FROM txn_hist_acct
        ...
        -- Get branch ID
        SELECT IFNULL(Branch_Code, 'DEFAULT') INTO v_branch_id
        FROM cust_acct_master
        WHERE Account_No = v_account_no
        LIMIT 1;
        
        IF v_branch_id IS NULL OR v_branch_id = 'DEFAULT' THEN
            SELECT IFNULL(Branch_Code, 'DEFAULT') INTO v_branch_id
            FROM of_acct_master
            WHERE Account_No = v_account_no
            LIMIT 1;
